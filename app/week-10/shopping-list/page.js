...
export default function Page() {
    ...
    const handleAddItem = async (newItem) => {
        try {
            const itemId = await addItem(user.uid, newItem); // Call addItem and get the ID
            const itemWithId = { ...newItem, id: itemId }; // Add the ID to the new item
            setItems((prevItems) => [...prevItems, itemWithId]); // Update state with new item
        } catch (error) {
